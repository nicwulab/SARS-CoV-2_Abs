...
plot_Vgene_usage <- function(data,ref,p_width,graphname){
  ...
  p <-  ggplot() +
          ...
          ylab("frequency (%)")
  ggsave(graphname,p,width=p_width, height=2, bg='white', dpi=1200)
}
plot_Dgene_usage <- function(data,ref,graphname){
  ...
  p <-  ggplot() +
          ...
          theme(axis.text=element_text(size=textsize,face="bold",colour = 'black'),
                ...
                legend.key.size = unit(0.3,"line")) +
          ylab("frequency (%)")
  ggsave(graphname,p,width=4, height=2, bg='white', dpi=1200)
}

set.seed(5)
HVgene_ref  <- read_excel('data/baseline_database/HV_Repertoire_freq.xlsx')
...
LVgene_ref  <- read_excel('data/baseline_database/LV_Repertoire_freq.xlsx')
...
Dgene_ref  <- read_excel('data/baseline_database/D_Repertoire_freq.xlsx')
